...
class TradingRunner {
    constructor(stockCodeArray) {
        this.stockCodes = stockCodeArray;

        this.client = createConnection();
        this.trader = new Trader(this.client, this.stockCodes);
    }

    rebalancePortfolio() {
        this.trader.nextTick();
    }
    ...
    run = async () => {
        ...
        var mainLoop = setInterval(async () => {
            ...
            if (getTimeTillMarketClose() < (60000 * 15)) {
                // Close all positions when 15 minutes till market close.
                console.log("Market closing soon. Closing positions.");

                sellAllPositions().catch((err) => { console.log(err.error); });
